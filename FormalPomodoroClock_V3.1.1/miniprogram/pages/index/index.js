...
Page({
  ...
  TimeSetting_Dynamic: function () {
    ...
    wx.createSelectorQuery()
      ...
      .exec((res) => {
        ...
        ////////////////////动态圆
        ctx.beginPath();
        ctx.arc(0, 0, width / 2 - 24, 0, that.data.Bud_Degree);
        ctx.lineWidth = 10;
        /*
        ...
        */
        ...
        //////////////////小纽扣
        ctx.beginPath();
        let Position_x = this.data.Bud_Position_x;
        let Position_y = this.data.Bud_Position_y;
  ...
  TouchConvas: function (e) {
    if (this.data.BudSelected && !this.data.IsStarted) {
      ...
      if (Math.abs(Math.PI / 6 - (Degree) % (Math.PI / 6)) <= 0.03) {
        that.setData({
          Bud_Position_x: Bud_Pos_x,
          Bud_Position_y: -Bud_Pos_y,
          Bud_Degree: Degree,
          SettedTime: ((that.data.Bud_Degree / Math.PI) * 6).toFixed(0) * 60 * 1000 * 5,
          TimeStr: ((that.data.Bud_Degree / Math.PI) * 6).toFixed(0) * 5 + ":00"
        })
