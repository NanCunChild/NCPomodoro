...
Page({
    ...
    wx.createSelectorQuery()
      ...
      .exec((res) => {
        ...
        */
        ...
        ctx.beginPath();
        let Position_x = that.data.Bud_Position_x;
        let Position_y = that.data.Bud_Position_y;
  ...
  TouchConvas: function (e) {
    ...
    if (this.data.BudSelected && !this.data.IsStarted) {
      ...
      var ratio = (that.data.ConvasWidth / 2 - 24) / Radius;
      ...
      // console.log("degree=",Degree,Bud_Pos_x,-Bud_Pos_y)
      if (Math.abs(Math.PI / 60 - (Degree) % (Math.PI / 6)) <= 0.33) { //调大一点容错率高
        that.setData({
          ...
          // SettedTime: ((that.data.Bud_Degree / Math.PI) * 6).toFixed(0) * 60 * 1000 * 5,//本来是五分钟改成1分钟
          SettedTime: ((that.data.Bud_Degree / Math.PI) * 30).toFixed(0) * 60 * 1000 * 1, //本来是五分钟改成1分钟
          TimeStr: ((that.data.Bud_Degree / Math.PI) * 30).toFixed(0) * 1 + ":00" //本来是5分钟改成1分钟
          // TimeStr: ((that.data.Bud_Degree / Math.PI) * 6).toFixed(0) * 5 + ":00"//本来是5分钟改成1分钟
        })
        // that.TimeSetting_Static();
        // var time=setInterval(function(){
      ...
      // console.log("(that.data.Bud_Degree).toFixed(2): " + (that.data.Bud_Degree).toFixed(2), "Math.PI / 30 :" + (Math.PI / 30).toFixed(2))
      // console.log("(that.data.Bud_Degree).toFixed(2) % (Math.PI / 30).toFixed(2)" + (that.data.Bud_Degree).toFixed(2) % (Math.PI / 6).toFixed(2))
      // console.log(that.data.SettedTime);
